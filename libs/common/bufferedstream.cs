namespace libs.common
{
    ...
    public class BufferedStream : Stream       /* http://msdn.microsoft.com/en-us/library/system.io.stream.aspx */
    {
        #region Constructors
        ...
        public BufferedStream(byte[] source)
        {
        ...
        public BufferedStream(int length)
        {
            SetLength(length);
            Position = length;
        ...
        public byte[] ToArray()
        {
            ...
            {
                GC.Collect();
                int buf = 4096; //4 KB //1024 * 1024; //// 1 MB
                byte[] buffer = new byte[buf];
                int read;
                do
                {
                    read = source.Read(buffer, 0, (int)Math.Min(buf, length));
                    length -= read;
